                    ...
                    <form method="POST" action="{{ route('agenda.store') }}" class="space-y-6">
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                            <div>
                                ...
                                <x-text-input id="data_inicio" name="data_inicio" type="datetime-local" class="mt-1 block w-full" :value="old('data_inicio')" required />
                                <x-input-error class="mt-2" :messages="$errors->get('data_inicio')" />
                            </div>

                            <div>
                                <x-input-label for="data_fim" :value="__('Data e Hora de Término')" />
                                <x-text-input id="data_fim" name="data_fim" type="datetime-local" class="mt-1 block w-full" :value="old('data_fim')" required />
                        ...
                        <div>
                            ...
                            <x-text-input id="local" name="local" type="text" class="mt-1 block w-full" :value="old('local')" />
                        ...
                        <div>
                            ...
                            <select id="tipo" name="tipo" class="mt-1 block w-full border-gray-300 focus:border-indigo-500 focus:ring-indigo-500 rounded-md shadow-sm">
                                ...
                                <option value="mentoria" {{ old('tipo') == 'mentoria' ? 'selected' : '' }}>Mentoria</option>
                        ...
                        <div>
                            ...
                            <select id="participantes" name="participantes[]" multiple class="mt-1 block w-full border-gray-300 focus:border-indigo-500 focus:ring-indigo-500 rounded-md shadow-sm">
                                @foreach(\App\Models\User::all() as $user)
                                    <option value="{{ $user->id }}" {{ in_array($user->id, old('participantes', [])) ? 'selected' : '' }}>
                                        {{ $user->name }}
                                    </option>
                                @endforeach
                            </select>
